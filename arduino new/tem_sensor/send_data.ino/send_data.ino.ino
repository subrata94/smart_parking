...
String server = "52.1.229.129"; // www.example.com //this is for thinhspeak.com
String uri = "/update?api_key=FEAUXFBZI6ZYK22R&field1="; //this is for thinhspeak.com

//String server = "192.168.0.101";//"arduino101.000webhostapp.com"; // www.example.com
//String uri = "/send.php?"; 
...
void connectWifi() {
  ...
  if (esp8266Module.find((char*)"OK")){
    ...
    esp8266Module.println("AT+CIPMUX=1");
    if (esp8266Module.find((char*)"OK") ) 
      Serial.println("CIPMUX SET 1");
	  delay(1000);
  }
  else {
...
void httppost () {
  esp8266Module.println("AT+CIPSTART=4,\"TCP\",\"" + server + "\",80");//start a TCP connection.
  if ( esp8266Module.find((char*)"OK")) {
    Serial.println("TCP connection ready");
  }
  /*String postRequest = "POST /send.php"+" HTTP/1.1\r\n" +
                       "Accept: *" + "/" + "*\r\n" +
 ...
 /*String postRequest =  "GET ";
  postRequest += uri;
  postRequest += "?cel="+cel;  //construct the http GET request
  postRequest += "&fer="+fer;
  postRequest += " HTTP/1.1\r\n";
  postRequest += "Host: ";
  postRequest += server;*/
  //postRequest += "\r\n\r\n";
  String postRequest = "GET "+uri+cel;//this is for thinhspeak.com
  //HTTP/1.1\r\nHost: api.pushingbox.com
  //String postRequest = "GET "+uri+"cel="+cel+"&fer="+fer;
  
  //String postRequest = "GET "+uri+D;
       
  Serial.println(postRequest);
  
  String sendCmd = "AT+CIPSEND=4,";    //determine the number of caracters to be sent.
  esp8266Module.print(sendCmd);
  int i = postRequest.length()+1;
  esp8266Module.println(i);
  delay(100);
  if (esp8266Module.find((char*)">")) {
    Serial.println("Sending..");
    esp8266Module.println(postRequest);
    delay(200);
